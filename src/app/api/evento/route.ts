...
const removerSenhasEventos = (eventos: any[]) => {
    return eventos.map((evento: any) => ({
        ...evento,
        eventoUsuario: evento.eventoUsuario?.map((eu: any) => ({
            ...eu,
            usuario: eu.usuario ? (() => {
                const { senha, email, ...usuarioSemDadosSensiveis } = eu.usuario;
                return usuarioSemDadosSensiveis;
            })() : null
        })) || [],
        eventoColaborador: evento.eventoColaborador?.map((ec: any) => {
            if (ec.colaborador && ('senha' in ec.colaborador || 'email' in ec.colaborador)) {
                const { senha, email, ...colaboradorSemDadosSensiveis } = ec.colaborador;
                return { ...ec, colaborador: colaboradorSemDadosSensiveis };
            }
            return ec;
        }) || []
    }));
...
export async function GET(request: Request) {
	...
	try {
		...
		if (titulo) {
			...
			// Remove senhas de todos os eventos
			const eventosSeguros = removerSenhasEventos(eventos);

			return NextResponse.json(
				eventosSeguros.map(c => ({
					...c,
		...
		// === Buscando eventos com id ===
			...
			if (evento) {
				// Remove senhas do evento único
				const eventoSeguro = removerSenhasEventos([evento])[0];
				
				return NextResponse.json({
					...eventoSeguro,
					categoriaFormatada: categoriaFormatada[eventoSeguro.categoria as keyof typeof categoriaFormatada] || eventoSeguro.categoria
				});
			} else {
