...
const removerSenhasProjetos = (projetos: any[]) => {
	return projetos.map((projeto: any) => ({
		...projeto,
		projetoUsuario: projeto.projetoUsuario?.map((pu: any) => ({
			...pu,
			usuario: pu.usuario ? (() => {
				const { senha, email, ...usuarioSemDadosSensiveis } = pu.usuario;
				return usuarioSemDadosSensiveis;
			})() : null
		})) || [],
		projetoColaborador: projeto.projetoColaborador?.map((pc: any) => {
			if (pc.colaborador && ('senha' in pc.colaborador || 'email' in pc.colaborador)) {
				const { senha, email, ...colaboradorSemDadosSensiveis } = pc.colaborador;
				return { ...pc, colaborador: colaboradorSemDadosSensiveis };
			}
			return pc;
		}) || []
	}));
...
export async function GET(request: Request) {
	...
	try {
		...
		if (titulo && categoria) {
			...
			// Remove senhas e emails dos projetos
			const projetosSeguros = removerSenhasProjetos(projetos);
			return NextResponse.json(
				projetosSeguros.map(p => ({
					...p,
		...
		// === Buscando projetos com id ===
			...
			if (projeto) {
				...
				// Remove senhas e emails dos projetos
				const projetosSeguro = removerSenhasProjetos([projeto])[0];
				return NextResponse.json({
					...projetosSeguro,
					categoriaFormatada: categoriaFormatada[projetosSeguro.categoria as keyof typeof categoriaFormatada] || projetosSeguro.categoria
				});
			} else {
